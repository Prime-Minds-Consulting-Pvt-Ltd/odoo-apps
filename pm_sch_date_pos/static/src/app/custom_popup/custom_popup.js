...
async processDates(datesArray) {
    ...
    try {
        ...
        if (!timeSlots || !timeSlots.length) {
            ...
            return datesArray.map(entry => ({ ...entry, selectable: false }));
        }
        ...
        endDate.setDate(currentDate.getDate() + max_schedule_days);
        ...
        return datesArray.map((entry) => {
            ...
            console.log(`Checking Date: ${entryDate.toDateString()}, Weekday: ${dayOfWeek}, In Range: ${isWithinRange}, Available: ${isAvailableWeekday}, Selectable: ${entry.selectable}`);
